...
CREATE OR REPLACE PROCEDURE gpn_oim.get_folder_tree_limit (
...
/* 
   edited : A.Kremer    18.11.2019      Remove duplicate sql query for calculation total count of folders, for optimization purposes in SUID 5;
                                        remove row with id = currentFolderId from query results.
            A.Kremer    15.05.2018      Add filters for is_deleted, is_hidden fields for quering z_resources and itsk_mv_resources_tree (redmine#695)   
*/
   ...
   SELECT res_tree_maxResults.*
     ...
     from (
        select r2.id row_key,  -- ����� ����� �� ������� gpn_oim.z_get_default_access_id
          ...
          from 
               (select id, name, description, owner_id, parent_resource_id, is_hidden, hc has_child, path_names, path_ids, uname, udescription
     ...
     group by r2.id, r2.name, r2.description, r2.owner_id, r2.parent_resource_id, r2.is_hidden, r2.has_child, usr.usr_display_name, r2.path_names, r2.path_ids
     order by r2.id, r2.name, r2.description, r2.owner_id, r2.parent_resource_id, r2.is_hidden, r2.has_child, usr.usr_display_name, r2.path_names, r2.path_ids
              ) res_tree_full
     where rownum <= (i_skip + i_maxResults) 
    ) res_tree_maxResults         
    WHERE rnum > i_skip;
    
END get_folder_tree_limit;
/

--rollback DROP PROCEDURE gpn_oim.get_folder_tree_limit;